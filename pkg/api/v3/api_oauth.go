...
/*
OauthApiService 刷新 Refresh Token
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param clientId
 * @param clientSecret
 * @param refreshToken
 * @param optional nil or *OauthRefreshTokenOpts - Optional Parameters:
     * @param "Fields" (optional.Interface of []string) -  返回参数的字段列表

@return OauthRefreshTokenResponse
*/

type OauthRefreshTokenOpts struct {
	Fields optional.Interface
}

func (a *OauthApiService) RefreshToken(ctx context.Context, clientId int64, clientSecret string, refreshToken string, localVarOptionals *OauthRefreshTokenOpts) (OauthRefreshTokenResponseData, http.Header, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Get")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarFileKey     string
		localVarReturnValue OauthRefreshTokenResponseData
		localVarResponse    OauthRefreshTokenResponse
	)
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v OauthRefreshTokenResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHttpResponse.Header, newErr
